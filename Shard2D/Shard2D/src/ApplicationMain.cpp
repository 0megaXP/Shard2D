...
{
    ShardGraphic->SetDefaultResolution(1280, 720, true);
    ShardGraphic->ResizeWindow(640, 360, WindowType::Window);

    ShardClock->EnableFrameRateCap(60);
    ...
    parent = new Sprite(ShardAssets->GetImageFromPNG("Assets/PNGs/Doge.png"));
    parent->x = 640;
    parent->y = 360;
    parent->rotation = 0;
    ...
    child = new Sprite(ShardAssets->GetImageFromPNG("Assets/PNGs/Doge.png"));
    ...
    grandChild = new Sprite(ShardAssets->GetImageFromPNG("Assets/PNGs/Doge.png"));
...
{
    fpsCounter->SetText("FPS: " + std::to_string(int(ShardClock->GetFPS())));

    runtime += ShardClock->GetDeltaTime();
